...
- **Prompt robustness**: ambiguous instructions, missing mechanical rules, subjective guidance that should be explicit, instruction ordering issues, weak emphasis on critical constraints, missing few-shot examples for classification/evaluation tasks, missing reasoning space (extended thinking or chain-of-thought) for multi-step tasks, real client/project/repo names appearing in examples (privacy leak + risks misleading the model into using example values as real config)
...
- **Agent/skill design**: scope clarity, tool selection, instruction specificity, input/output contracts, orchestration patterns, loop patterns with exit conditions, graceful degradation, cost/latency concerns. For Claude Code skills: in headless `claude --print` mode, `settings.json`'s `permissions.allow` is not consulted for tool gating — `--allowed-tools` is required and only reliably accepts simple tool names. Flag skills that assume per-command settings.json gating in headless invocation.
